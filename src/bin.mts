...
const { version: binVersion } = loadPackageJson(
  import.meta.url,
  '../package.json',
)

const { version: libVersion } = globPkg
...
const j = jack({
  ...
  .description(
    `
    Glob v${libVersion}, CLI v${binVersion}
  ...
  .flag({
    ...
    version: {
      short: 'V',
      description: `Output the glob library version (${libVersion})`,
    },
    help: {
      short: 'h',
